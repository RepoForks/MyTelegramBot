...
def search_photo_and_reply(message, pic):
    ...
    try:
        ...
        bot.send_message(message.chat.id, '搜图快速入口：\n<a herf=\"{0}\">Google</a>'.format(bot_utils.get_google_search_image(public_url)), reply_to_message_id=message.message_id, disable_web_page_preview = True, parse_mode = 'HTML')
    except:
        traceback.print_exc()
        bot.reply_to(message, '出现了点意外，等会再试试吧……')
